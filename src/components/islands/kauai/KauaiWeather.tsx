...
export const KauaiWeather = () => {
  ...
  const kauaiWeather = weatherData["Kauai"];
  ...
  if (error.length > 0 && !kauaiWeather) {
    return (
      ...
      <CardHeader>
        <CardTitle className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            {kauaiWeather ? getConditionIcon(kauaiWeather.condition) : <Sun className="w-5 h-5 text-sunset-100" />}
            Current Weather in Kauai
          </div>
          <button
            onClick={refreshWeather}
            disabled={loading}
        ...
        <CardDescription className="flex items-center gap-2">
          ...
          {kauaiWeather && (
            <>
              <span>•</span>
              <span>{kauaiWeather.location}</span>
            </>
          )}
        </CardDescription>
      </CardHeader>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              ...
              <div className="text-center md:text-left">
                <div className="flex items-center gap-3 mb-2">
                  <span className="text-4xl">{getWeatherEmoji(kauaiWeather.condition)}</span>
                  <div>
                    <div className="text-3xl font-bold text-gray-900">
                      {kauaiWeather.temperature}°F
                    </div>
                    <div className="text-lg text-gray-600">
                      {kauaiWeather.temperatureCelsius}°C
                    </div>
                  </div>
                </div>
                <div className="text-xl font-semibold text-gray-800 mb-1">
                  {kauaiWeather.condition}
                </div>
                <div className="text-sm text-gray-600">
                  {kauaiWeather.description}
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-white/80 backdrop-blur-sm rounded-lg p-3 border border-gray-200">
                  ...
                  <div className="text-lg font-bold text-gray-900">{kauaiWeather.humidity}%</div>
                </div>
                ...
                <div className="bg-white/80 backdrop-blur-sm rounded-lg p-3 border border-gray-200">
                  ...
                  <div className="text-lg font-bold text-gray-900">
                    {kauaiWeather.windSpeed} mph
                  </div>
                  <div className="text-xs text-gray-600">{kauaiWeather.windDirection}</div>
                </div>
                ...
                <div className="bg-white/80 backdrop-blur-sm rounded-lg p-3 border border-gray-200">
                  ...
                  <div className="text-lg font-bold text-gray-900">{kauaiWeather.pressure}"</div>
                ...
                <div className="bg-white/80 backdrop-blur-sm rounded-lg p-3 border border-gray-200">
                  ...
                  <div className="text-lg font-bold text-gray-900">{kauaiWeather.visibility}</div>
            ...
            <div className="border-t border-gray-200 pt-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                <div>
                  ...
                  <span className="font-semibold ml-2">{kauaiWeather.dewPoint}°F</span>
                </div>
                {kauaiWeather.heatIndex && (
                  <div>
                    <span className="text-gray-600">Heat Index:</span>
                    <span className="font-semibold ml-2">{kauaiWeather.heatIndex}°F</span>
                  </div>
                )}
                <div>
                  <span className="text-gray-600">Updated:</span>
                  <span className="font-semibold ml-2">
                    {kauaiWeather.lastUpdated.toLocaleTimeString()}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="mt-4 pt-4 border-t border-gray-200">
              <div className="flex items-center justify-between text-xs text-gray-500">
                <div className="flex items-center gap-1">
                  ...
                  <span>Weather Station: {kauaiWeather.location}</span>
                </div>
                <div>
